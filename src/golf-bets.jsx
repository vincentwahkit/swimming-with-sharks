...
<body>
  <div class="header">
    ...
    <div class="header-right" style="color:#e8f5e8">
      ...
      <div style="margin-top:2px;color:#4a7a4a">vw-0.9.4</div>
    </div>
  </div>
  ...
  <div class="footer">
    Generated by Swimming With Sharks vw-0.9.4 · ${new Date().toLocaleString("en-SG")}
  </div>
    ...
    <div style={S.page}>
      ...
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "0 0 40px" }}>
        ...
        <div style={{ position: "relative", textAlign: "center", padding: "28px 20px 16px", background: "linear-gradient(180deg, #0d2a0d 0%, #0a1a0a 100%)" }}>
          <div style={{ position: "absolute", top: 8, right: 12, fontSize: 10, color: "#5a8a5a", fontFamily: "'DM Sans', sans-serif", letterSpacing: 1 }}>vw-0.9.4</div>
        ...
        {view === "hole" && (
          <div style={{ display: "flex", gap: 2, padding: "0 8px 8px" }}>
            {Array.from({length:18}, (_,i) => {
              ...
              const bg = isCurrent ? COLORS[0] : isPlayed ? (isPar3 ? "#1a4a6a" : "#2a5a2a") : (isPar3 ? "#0d2a3a" : "#1e3a1e");
              const col = isCurrent ? "#0a1a0a" : isPlayed ? (isPar3 ? "#60a5fa" : COLORS[0]) : (isPar3 ? "#2a5a7a" : "#3a5a3a");
              return (
                <div key={i} onClick={() => { setHoleIdx(i); setView("hole"); }}
                  style={{ flex: 1, height: 20, borderRadius: 3, cursor: "pointer",
                    ...
                    border: isPar3 ? `1px solid ${isCurrent?COLORS[0]:"#1a4a6a"}` : "none",
            ...
            <Sect title="Players">
              {[0,1,2,3].map(pi => (
                <div key={pi} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
                  <div style={{ ...S.dot, background: COLORS[pi], flexShrink: 0 }}>{liveNames[pi][0]}</div>
          ...
          <>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 6, marginBottom: 10 }}>
                {[0,1,2,3].map(pi => (
                  <div key={pi} style={{ textAlign: "center" }}>
                    <div style={{ color: COLORS[pi], fontWeight: "600", fontSize: 14, fontFamily: "'DM Sans', sans-serif", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{liveNames[pi]}</div>
                    {(() => {
            ...
            {games.vegas && <div style={{ opacity: inPlay[holeIdx] ? 1 : 0.4, pointerEvents: inPlay[holeIdx] ? "auto" : "none" }}><Sect title="Vegas — Teams">
              <div style={{ fontSize: 12, color: "#4a7a4a", marginBottom: 8 }}>
                Pick <span style={{ color: COLORS[0], fontWeight: "600" }}>{liveNames[0]}</span>'s partner
              </div>
              <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                {[1,2,3].map(pi => {
              ...
              <div style={{ fontSize: 11, color: "#4a7a4a", marginBottom: 8 }}>
                <span style={{ color: COLORS[0] }}>{liveNames[0]}</span>+<span style={{ color: COLORS[vTeams[holeIdx][0][1]] }}>{liveNames[vTeams[holeIdx][0][1]]}</span>
                {" "}<span style={{ color: "#2a4a2a" }}>vs</span>{" "}
                <span style={{ color: COLORS[vTeams[holeIdx][1][0]] }}>{liveNames[vTeams[holeIdx][1][0]]}</span>+<span style={{ color: COLORS[vTeams[holeIdx][1][1]] }}>{liveNames[vTeams[holeIdx][1][1]]}</span>
              </div>
              {res.vr && (() => {
                const r = res.vr;
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    {[0,1,2,3].map(pi => (
                      <button key={pi} style={{ flex: 1, padding: "16px 0", borderRadius: 8, cursor: "pointer",
                        ...
                        <div style={{ fontSize: 11, color: p3mult[holeIdx][pi]>1?COLORS[pi]:"#4a7a4a", marginBottom: 2 }}>{liveNames[pi]}</div>
                        <div style={{ fontSize: 28, fontWeight: "700", color: p3mult[holeIdx][pi]>1?COLORS[pi]:"#4a7a4a", lineHeight: 1, fontFamily: "'Bebas Neue', sans-serif" }}>×{p3mult[holeIdx][pi]}</div>
                      </button>
                    ))}
                  </div>
                  {/* Effective matchup multipliers */}
                  <div style={{ marginTop: 10, background: "#071507", borderRadius: 8, padding: "8px 10px", border: "1px solid #1e3a1e" }}>
                    <div style={{ fontSize: 10, color: "#4a7a4a", letterSpacing: 2, marginBottom: 6, fontFamily: "'DM Sans', sans-serif" }}>STAKES PER MATCHUP</div>
                        ...
                        return (
                          <div key={pi} style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                            <span style={{ fontSize: 12, color: "#aaa", fontFamily: "'DM Sans', sans-serif" }}>
                              <span style={{ color: COLORS[banker[holeIdx]] }}>{liveNames[banker[holeIdx]]}</span>
                              {" vs "}
                              <span style={{ color: COLORS[pi] }}>{liveNames[pi]}</span>
                            </span>
